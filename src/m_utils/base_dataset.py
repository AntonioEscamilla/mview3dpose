...
class PreprocessedDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for cam_id, info_dict in info_dicts.items():
            # image name format: 'dataset-frame-c0.jpg'
            # cam_folder = 'Camera' + osp.basename(info_dict['image_path']).split('-')[-1][1]
            cam_folder = 'Camera' + str(cam_id)
            info_dict['image_data'] = cv2.cvtColor(cv2.imread(osp.join(dump_dir, cam_folder, info_dict['image_path'])), cv2.COLOR_BGR2RGB)
